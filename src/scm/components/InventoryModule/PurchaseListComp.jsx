...
class PurchaseListComp extends Component {
    constructor(props, context) {
        ...
        columns[2].render = (txt, record, index) =><a onClick={()=>this.props.openSideBar(record.sourceOrderCode)} className="purchase-color-href">{record.sourceOrderCode}
        </a>
        columns[4].render = (txt, record, index) => <TooltipComp attr={{text: <span onClick={()=>this.props.openSideBarSub({supplierCode:record.bpCode,langCode:record.langCode})} className="purchase-color-href">{record.bpName}</span>, wid: 140}} />
...
componentDidMount() {
        this.props.PurchaseList({page:1,pageSize:15,sourceOrderType:10});
}
searchBar=()=>{//查询按钮
     let  searchVal = this.props.form.getFieldsValue();
     this.props.PurchaseList({page:1, pageSize:this.props.newState.paging.pageSize,sourceOrderType:10,...searchVal})
