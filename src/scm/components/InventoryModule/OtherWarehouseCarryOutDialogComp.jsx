...
class TableComp extends EditableTableComp {
        ...
        this.columns = [
            {
                ...
                title: '仓库',
                dataIndex: 'stockName',   // warehouseName
                key: 'stockName',
                width: 100
            }, {
                title: '仓库编码',
                dataIndex: 'warehouseCode',  // warehouseCode
                key: 'warehouseCode',
                hidden: true,
            }, {
                title: '批次号',
    ...
    handleChange = (key, index, value) => {
        if (key === 'freightSpaceCode') {
            ...
            this.record.stockName = value.stockName;
            this.record.warehouseCode = value.stockId || this.record.warehouseCode;
            //--- 为了二次编辑时，下拉框中能有当前行数据显示
            this.record.code = value.code;
            this.record.name = value.name;
            // this.record.stock = value.stockName; 
        }
        if (key === 'batchNum') {
        ...
        return (
            <div className='pre-receipt-dialog'>
                ...
                <MTable
                    ...
                    pagination={false}
                    record={record}
                    className='otherwarehouse-dialog'
                />
            </div>
        )
    }
