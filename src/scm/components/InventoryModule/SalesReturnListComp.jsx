...
class SalesReturnListComp extends Component {
    constructor(props, context) {
        ...
        columns[2].render = (txt, record, index) =><a className="purchase-color-href" onClick={()=>this.props.openSideBar(record.sourceOrderCode)}>{record.sourceOrderCode}
        </a>
        columns[4].render = (txt, record, index) =><TooltipComp attr={{text: <span className="purchase-color-href" onClick={()=>this.props.openSideBarSub({customerCode:record.bpCode,langCode:record.langCode})}>{record.bpName}</span>, wid: 140}} />
...
componentDidMount() {
           this.props.PurchaseList({page:1,pageSize:15,sourceOrderType:32});
}
searchBar=()=>{//查询按钮
