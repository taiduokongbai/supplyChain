...
import { addPurRecNoticeStore, addPurRecNoticeDetailStore } from '../stores/AddPurRecNoticeStore';
...
@observer
export default class AddPurRecNoticeDetailComp extends Component {
    constructor(props, context) {
        super(props, context);
        this.store = addPurRecNoticeDetailStore;
        this.store.onSave = this.onSave;
        this.purFormStore = addPurRecNoticeStore;
        this.columns = [
            {
                title: '行号',
                dataIndex: 'lineNo',
                ...
                title: '收货数量',
                dataIndex: 'endReceiveQty',
                key: 'endReceiveQty',
                width: 84,
                obj: {
                    rules: [
                        { type: 'gtEqZero', label: '收货数量' }
                    ],
                    render: (text, record, index) => Number(text).toFixed(2),
                },
            }, {
    ...
    render() {
        ...
        return (
            <div className='purRecNotice-table'>
                <MyTableEditCont
                    {...Props}
