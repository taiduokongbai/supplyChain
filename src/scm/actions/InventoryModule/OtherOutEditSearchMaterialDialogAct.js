...
const actions = {
    show: () => (dispatch, getState) => {
        let state = getState()[OTHER_OUT_EDIT_SEARCH_MATERIAL_DIALOG_REDU].set("visible", true);

        dispatch({ type: OTHER_OUT_EDIT_SEARCH_MATERIAL_DIALOG_REDU, state });
    },
    hide: () => (dispatch, getState) => {
        let state = getState()[OTHER_OUT_EDIT_SEARCH_MATERIAL_DIALOG_REDU].set("visible", false);
        dispatch({ type: OTHER_OUT_EDIT_SEARCH_MATERIAL_DIALOG_REDU, state });
    },
    tableLoading: (val) => (dispatch, getState) => {
        let state = getState()[OTHER_OUT_EDIT_SEARCH_MATERIAL_DIALOG_REDU].set('tableLoading', val);
        dispatch({ type: OTHER_OUT_EDIT_SEARCH_MATERIAL_DIALOG_REDU, state })
    },
    btnLoading: (val) => (dispatch, getState) => {
        let state = getState()[OTHER_OUT_EDIT_SEARCH_MATERIAL_DIALOG_REDU].set('btnLoading', val);
        dispatch({ type: OTHER_OUT_EDIT_SEARCH_MATERIAL_DIALOG_REDU, state })
    },
    GetPurchaseList: (data, pm) => (dispatch, getState) => {
        let { list, total, page, pageSize } = data;
        let state = getState()[OTHER_OUT_EDIT_SEARCH_MATERIAL_DIALOG_REDU].set('search', pm).set('dataSource', list)
    ...
    checkedTableList: (val) => (dispatch, getState) => {
        let state = getState()[OTHER_OUT_EDIT_SEARCH_MATERIAL_DIALOG_REDU].setIn(['addWareHouse', 'checkedTableList'], val);
        dispatch({ type: OTHER_OUT_EDIT_SEARCH_MATERIAL_DIALOG_REDU, state });
    }
}
