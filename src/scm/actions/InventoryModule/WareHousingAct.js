...
import { WAREHOUSINGREDU, WAREHOUSINGDETAILSREDU, ADDWAREHOUSINGREDU } from '../../consts/ActTypes'
...
const actions = {
    loading: (loading, value) => (dispatch, getState) => {
        let state = getState()[WAREHOUSINGREDU].set(loading, value);
        dispatch({ type: WAREHOUSINGREDU, state });
    },
    // openSideBar: (value, pm = {}) => (dispatch, getState) => {
    //     dispatch(actions.loading('openSideBarLoading', true))
    //     let state = getState()[WAREHOUSINGREDU].set('side_visible', value);
    //     dispatch({ type: WAREHOUSINGREDU, state })
    //     if (value == true) {
    //         return ReqApi.get({
    ...
    //         }).then((json) => {
    //             if (json.status === 2000) {
    //                 let state = getState()[WAREHOUSINGREDU].set('dataSourceSide', json.data.list).set('openSideBarList', json.data);
    //                 dispatch({ type: WAREHOUSINGREDU, state });
    //             }
    ...
    //     let state = getState()[WAREHOUSINGREDU].set('side_visible_sub', value).set('suplierData', pm);
    //     dispatch({ type: WAREHOUSINGREDU, state })
    //     // dispatch(actions.loading('openSideBarSubLoading',false))
    // },
    takeOrderPm: (pm = {}) => (dispatch, getState) => {//执行传参
        let state = getState()[WAREHOUSINGREDU].set('takeOrderPm', pm);
        dispatch({ type: WAREHOUSINGREDU, state })
    },
    ...
    takeOrderDetailsPm: (pm = {}) => (dispatch, getState) => {//详情传参获取主界面信息
        return ReqApi.get({
            ...
            if (json.status === 2000) {
                let state = getState()[WAREHOUSINGDETAILSREDU].set('takeOrderDetailsPm', json.data);
                dispatch({ type: WAREHOUSINGDETAILSREDU, state })
            }
            return json;
        });
    },
    takeOrderDetailsListPm: (pm = {}) => (dispatch, getState) => {//详情传参获取收货记录信息
        return ReqApi.get({
            ...
            if (json.status === 2000) {
                let state = getState()[WAREHOUSINGDETAILSREDU].set('takeOrderDetailsListPm', json.data);
                dispatch({ type: WAREHOUSINGDETAILSREDU, state })
            }
            return json;
        });
    ...
    takeOrderDetailsMaterialPm: (pm = {}) => (dispatch, getState) => {//详情传参获取物料列表查询
        ...
        return ReqApi.get({
            ...
            if (json.status === 2000) {
                let state = getState()[WAREHOUSINGDETAILSREDU].set('search', pm).set('dataSource', json.data.list).set('paging', { total: json.data.total, current: json.data.page, pageSize: json.data.pageSize });
                dispatch({ type: WAREHOUSINGDETAILSREDU, state });
            }
    ...
    materialLoading: (val) => (dispatch, getState) => {//详情传参获取物料列表查询Loading
        let state = getState()[WAREHOUSINGDETAILSREDU].set('materialLoading', val);
        dispatch({ type: WAREHOUSINGDETAILSREDU, state })
    },
    setLoading: (bool) => (dispatch, getState) => {//详情传参loading
        let state = getState()[WAREHOUSINGDETAILSREDU].set('DetailsPmLoading', bool);
        dispatch({ type: WAREHOUSINGDETAILSREDU, state })
    },
    takeOrderDelete: (pm = {}) => (dispatch, getState) => {//删除销售出库单行
        ...
        return ReqApi.get({
            ...
            if (json.status === 2000) {
                dispatch(actions.PurchaseList(getState()[WAREHOUSINGREDU].get('search')));
                return json;
            } else {
    ...
    tableLoading: (val) => (dispatch, getState) => {
        let state = getState()[WAREHOUSINGREDU].set('tableLoading', val);
        dispatch({ type: WAREHOUSINGREDU, state })
    },
    GetPurchaseList: (data, pm) => (dispatch, getState) => {
        let { list, total, page, pageSize } = data;
        let state = getState()[WAREHOUSINGREDU].set('search', pm).set('dataSource', list)
    ...
    GetIslock: (pm = {}) => (dispatch, getState) => {
        ReqApi.get({
            ...
            if (json.status === 2000 && json.data.isLock == 0) {
                ...
                dispatch(ProductionStorageTopAct.getInitialData());
                dispatch(TabsAct.TabAdd({ title: '执行生产入库单', key: 'inventoryProductionStorageCont' }));
    ...
    btnLoading: (val) => (dispatch, getState) => {
        let state = getState()[WAREHOUSINGREDU].set('btnLoading', val);
        dispatch({ type: WAREHOUSINGREDU, state })
    },
}
