...
export let setVisible =  (bool) => (dispatch,getState)=>{
    let state =  getState()[OTHER_OUT_EDIT_ROW_DIALOG_REDU].setIn(["visible"],bool);
    dispatch({type:OTHER_OUT_EDIT_ROW_DIALOG_REDU, state});
}
export let setLoading =  (bool) => (dispatch,getState)=>{
    let state =  getState()[OTHER_OUT_EDIT_ROW_DIALOG_REDU].setIn(["loading"],bool);
    dispatch({type:OTHER_OUT_EDIT_ROW_DIALOG_REDU, state});
}

export let setDataSource =  (dataSource={}) => (dispatch,getState)=>{
    let state =  getState()[OTHER_OUT_EDIT_ROW_DIALOG_REDU].setIn(["dataSource"],dataSource);
    dispatch({type:OTHER_OUT_EDIT_ROW_DIALOG_REDU, state});
}
